...
<body>
    ...
    <div class="container">
        <ul class="nav nav-pills">
            <li class="nav-item">
                <a class="nav-link  ${ clickType == 0 ? "active" :""}" href="/topicList">全部</a>
            </li>
            <c:forEach items="${courseTypeList}" var="type">
                <li class="nav-item">
                    <a class="nav-link ${ type.id == clickType ? "active" :""} " href="/topicList/${type.id}">${type.name}</a>
                </li>
            </c:forEach>
        </ul>
        <%-- - 0[row] 1 2 3[row] - --%>
            ...
            <div class="col mb-3">
                <a href="/topic/${topic.id}" target="_blank">
                    <div class="card select-shadow">
                        <img src="http://localhost:8888/static/img/hh.jpg" class="card-img-top" alt="...">
                        <div class="card-body">
        ...
        <ul class="pagination justify-content-center">
            <li class="page-item ${topicList.hasPreviousPage? "" :"disabled"} ">
                <a class="page-link" href="/topicList/${clickType}?pageNum=${topicList.prePage}">上一页</a>
            </li>
            <%--            //导航条上的第一页--%>
            ...
            <c:forEach var="i" begin="${topicList.navigateFirstPage}" end="${topicList.navigateLastPage}">
                <li class=" page-item ${topicList.pageNum == i ? "active" : "" } ">
                    <a class="page-link" href="/topicList/${clickType}?pageNum=${i}">${i}</a>
                </li>
            </c:forEach>
            <li class="page-item ${topicList.hasNextPage? "" :"disabled"} ">
                <a class="page-link" href="/topicList/${clickType}?pageNum=${topicList.nextPage}">下一页</a>
            </li>
        </ul>
    </nav>
    </div>
